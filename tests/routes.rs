...
use srvcs_and::{health, router, telemetry};
...
#[tokio::test]
async fn post_true_and_true_is_true() {
    let (status, body) = post_eval(serde_json::json!({ "a": true, "b": true })).await;
...
#[tokio::test]
async fn post_true_and_false_is_false() {
...
#[tokio::test]
async fn post_false_and_false_is_false() {
...
#[tokio::test]
async fn post_non_boolean_a_is_rejected() {
    let (status, body) = post_eval(serde_json::json!({ "a": 1, "b": true })).await;
    assert_eq!(status, StatusCode::UNPROCESSABLE_ENTITY);
    assert!(body["error"].is_string());
}

#[tokio::test]
async fn post_non_boolean_b_is_rejected() {
    let (status, _) = post_eval(serde_json::json!({ "a": true, "b": "true" })).await;
    assert_eq!(status, StatusCode::UNPROCESSABLE_ENTITY);
}
